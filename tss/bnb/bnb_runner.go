...
import (
	...
	"github.com/BroLabel/brosettlement-mpc-core/tss/bnb/flow"
...
type BnbRunner struct {
	...
	logger    *slog.Logger
	debug     bool
	cfg       tssutils.RunnerConfig
	metrics   bnbutils.Metrics
}

func NewBnbRunner(logger *slog.Logger) *BnbRunner {
	return newBnbRunner(logger, bnbutils.NoopMetrics{}, tssutils.LoadRunnerConfigFromEnv())
}

func NewBnbRunnerWithMetrics(logger *slog.Logger, metrics bnbutils.Metrics) *BnbRunner {
	return newBnbRunner(logger, metrics, tssutils.LoadRunnerConfigFromEnv())
}

func newBnbRunner(logger *slog.Logger, metrics bnbutils.Metrics, cfg tssutils.RunnerConfig) *BnbRunner {
	if logger == nil {
		logger = slog.Default()
	}
	if metrics == nil {
...
func (r *BnbRunner) ECDSAAddress(key string) (string, error) {
	...
	addr, err := tssutils.ECDSAAddressFromShare(share)
	if errors.Is(err, tssutils.ErrECDSAPubKeyUnavailable) {
